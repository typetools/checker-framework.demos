...
public final class UtilMDE {
  ...
  public static Class<?> classForName(String className) throws ClassNotFoundException {
    @Nullable Class<?> result = primitiveClasses.get(className);
    if (result != null) {
      return result;
    } else {
  ...
  public static String classnameToJvm(String classname) {
    ...
    @Nullable String result = primitiveClassesJvm.get(classname);
    if (result == null) {
      result = "L" + classname + ";";
    }
   ...
   **/
  public static @Nullable String arglistFromJvm(String arglist) {
    if (! (arglist.startsWith("(") && arglist.endsWith(")"))) {
      throw new Error("Malformed arglist: " + arglist);
    }
    String result = "(";
  ...
  public static boolean equalFiles(String file1, String file2, boolean trimLines) {
    try {
      ...
      @Nullable String line1 = reader1.readLine();
      @Nullable String line2 = reader2.readLine();
  ...
  public static boolean canCreateAndWrite(File file) {
    if (file.exists()) {
      ...
      @Nullable File directory = file.getParentFile();
      if (directory == null) {
        directory = new File(".");
      }
  ...
  public static <T> Integer incrementHashMap(HashMap<T,Integer> hm, T key, int count) {
    @Nullable Integer old = hm.get(key);
    int new_total;
    if (old == null) {
      new_total = count;
    } else {
  ...
  */
  ...
  public static Method methodForName(String method)
    ...
    @SuppressWarnings("nullness") @NonNull Class<?>[] argclasses = args_seen.get(all_argnames);
    if (argclasses == null) {
      String[] argnames;
  ...
  public static boolean propertyIsTrue(Properties p, String key) {
    @Nullable String pvalue = p.getProperty(key);
    if (pvalue == null) {
      return false;
    }
  ...
  public static @Nullable String appendProperty(Properties p, String key, String value) {
    return (@Nullable String)p.setProperty(key, p.getProperty(key, "") + value);
  }
  ...
  public static @Nullable String setDefault(Properties p, String key, String value) {
    @Nullable String currentValue = p.getProperty(key);
...
// Add to deleteDir() :
//    if (files == null) throw new IllegalArgumentException("dir is not a directory");
